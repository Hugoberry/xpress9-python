...
// C code: 26.3 MB/s, asm code: 32.3 MB/s at P3-500
  ...
  Mask = * (UNALIGNED ubitmask2 *) src; src += sizeof (ubitmask2);
  ...
  LABEL(pointer):
    ...
    if (len <= 8-MIN_MATCH)
    {
      ...
      if (ofs < ~2)
      {
        ...
        ofs = ((UNALIGNED uint32 *) src)[0];   // copy 8 bytes
        ((UNALIGNED uint32 *) dst)[0] = ofs;
        ofs = ((UNALIGNED uint32 *) src)[1];
        ((UNALIGNED uint32 *) dst)[1] = ofs;
...
LABEL (next):
  ...
  if (len <= 8 - MIN_MATCH)
  {
    ...
    if (ofs < ~2)
    {
      ...
      ofs = ((UNALIGNED uint32 *) src1)[0];     // quickly copy 8 bytes
      ((UNALIGNED uint32 *) dst)[0] = ofs;
      ofs = ((UNALIGNED uint32 *) src1)[1];
      ((UNALIGNED uint32 *) dst)[1] = ofs;
