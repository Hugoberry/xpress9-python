...
{                                                \
  ...
  {                                              \
    *(UNALIGNED tag_t *) v.temp.tag_ptr = __n; \
    v.temp.tag_ptr = ptr;                        \
...
#define tag_write(p,ptr,n) do { \
  if (--v.stat.bits < 0) \
  { \
    *(UNALIGNED tag_t *)v.temp.tag_ptr = v.temp.tag_mask; \
    v.temp.tag_mask = n; \
...
#define tag_write_mask(p,ptr,n,b) do { \
  if ((v.stat.bits -= (b)) < 0) \
  { \
    *(UNALIGNED tag_t *)v.temp.tag_ptr = (v.temp.tag_mask << ((b) + v.stat.bits)) \
      + ((n) >> (-v.stat.bits)); \
...
#define BIOWR(mask,bits) {                                      \
  ...
  if ((Bits -= (bits)) < 0)                                     \
  {                                                             \
    *(UNALIGNED bitmask2 *)Ptr1 = (bitmask2) ((Mask << (Bits + (bits))) \
      + ((mask) >> (-Bits)));                                   \
    Mask = (mask);                                              \
...
next:
  ...
  {
  start:
    *(UNALIGNED tag_t *)ptr = bmask = *(UNALIGNED tag_t *)src;
    src += sizeof (tag_t);
    ptr += sizeof (tag_t);
    if (bmask >= 0)
    {
...
next:
  ...
  {
  start:
    bmask = *(UNALIGNED tag_t *)src;
    src += sizeof (tag_t);
#if CODING == CODING_HUFF_PTR
    *(UNALIGNED tag_t *)ptr = bmask;
    ptr += sizeof (tag_t);
#endif
    if (bmask >= 0)
    {
...
#endif
  *(UNALIGNED bitmask2 *)Ptr1 = (ubitmask2) (Mask <<= Bits);
  *(UNALIGNED bitmask2 *)Ptr2 = 0;
  v.comp.ptr = ptr;
  assert (src == v.temp.ptr);
}
