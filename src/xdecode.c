...
#define GET_UINT16(x,p) x = *(UNALIGNED uint16 *)(p); p += 2
...
#if 0
#define COPY_BLOCK_FAST_8(dst,src) \
  ((UNALIGNED uint32 *) dst)[0] = ((UNALIGNED uint32 *) src)[0]; \
  ((UNALIGNED uint32 *) dst)[1] = ((UNALIGNED uint32 *) src)[1]
#else
#define COPY_BLOCK_FAST_8(dst,src) \
  ((UNALIGNED __int64 *) dst)[0] = ((UNALIGNED __int64 *) src)[0]
#endif
#endif /* i386 */
...
#define BIORD_MORE0(bits)                       \
  ...
  {                                             \
    ...
    Mask += ((ubitmask4) *(UNALIGNED ubitmask2 *)src) << (-Bits); \
